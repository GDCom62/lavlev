...
try:
    if "banco_dados" in st.secrets and "uri" in st.secrets["banco_dados"]:
        db_conexao = psycopg2.connect(st.secrets["banco_dados"]["uri"])
except Exception as e:
    st.error(f"⚠️ Erro na conexão inicial do banco: {e}")

# --- INTERFACE LATERAL (MENU UNIFICADO) ---
...
if menu == "Lavagem":
    ...
    if st.button("Gravar Lavagem") and db_conexao:
        try:
...
if menu == "Lavados":
    ...
    i = st.text_input("Horário Início", key="lvd_i")
    t = st.text_input("Horário Término", key="lvd_t")
    e = st.text_input("Executante", key="lvd_e")
    if st.button("Gravar Lavados") and db_conexao:
        try:
            cursor = db_conexao.cursor()
...
if menu == "Secagem":
    ...
    if st.button("Gravar Secagem") and db_conexao:
        try:
            cursor = db_conexao.cursor()
...
if menu == "Pesagem":
    ...
    if st.button("Gravar Pesagem") and db_conexao:
        try:
            cursor = db_conexao.cursor()
...
if menu == "Dobragem":
    ...
    if st.button("Gravar Dobragem") and db_conexao:
        try:
...
if menu == "📊 Resumos e Análises" and db_conexao:
...
# --- FLUXO 7: GERENCIAMENTO E HISTÓRICO (LISTAGEM, FILTROS, EDIÇÃO E EXCLUSÃO) ---
if menu == "🛠️ Histórico" and db_conexao:
    ...
    try:
        ...
        df_dados = pd.read_sql_query(query_hist, db_conexao, params=params_hist)
                    ...
                    if st.button("💾 CONFIRMAR E SALVAR ALTERAÇÕES NO BANCO"):
                        ...
                        if deletadas:
                            for indice in deletadas:
                                id_reg = int(df_dados.iloc[indice]["id"])
