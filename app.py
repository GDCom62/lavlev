    ...
    if credenciais_dict is not None:
        try:
            if "private_key" in credenciais_dict:
                pk = credenciais_dict["private_key"].strip().strip('"').strip("'").replace("\\n", "\n")
                conteudo_puro = pk.replace("-----BEGIN PRIVATE KEY-----", "").replace("-----END PRIVATE KEY-----", "").replace("\n", "").replace("\r", "").replace(" ", "")
                linhas = [conteudo_puro[i:i+64] for i in range(0, len(conteudo_puro), 64)]
...
def deletar_ultima_linha(setor):
    planilha = conectar_sheets()
    if planilha is None: st.error("❌ Envie o arquivo .json primeiro."); return
    try:
...
def puxar_historico_setor(setor):
    planilha = conectar_sheets()
    if planilha is None: st.info("Envie o arquivo .json para ver o histórico."); return
    try:
        dados = planilha.worksheet(setor).get_all_records()
        if not dados: st.info("Aba vazia.")
        else: st.dataframe(pd.DataFrame(dados).tail(10), use_container_width=True)
    except Exception as e: st.error(f"Erro: {e}")

def gerar_relatorios(filtro_cliente):
    planilha = conectar_sheets()
    if planilha is None: st.info("Envie o arquivo .json para ver os relatórios."); return
    try:
        df_geral = []
        for s in ["Lavagem", "Lavados", "Secagem", "Pesagem", "Dobragem"]:
            ...
            if dados:
                ...
                df["Setor"] = s
                if "Executante" in df.columns: df_geral.append(df[["Executante", "Setor"]])
        ...
        if dados_dob:
            ...
            for it in ITENS_DOBRAGEM:
                if it in df_dob.columns: df_dob[it] = pd.to_numeric(df_dob[it], errors='coerce').fillna(0)
        ...
        else: st.info("Nenhum registro em Dobragem.")
...
def pag_lavagem(dt):
    ...
    with st.form("f_lav", clear_on_submit=True):
        ...
        if st.form_submit_button("Gravar Lavagem"):
            if c and e: registrar_dados("Lavagem", [c, dt, m, p, i, t, e])
...
def pag_lavados(dt):
    ...
    with st.form("f_lvd", clear_on_submit=True):
        ...
        if st.form_submit_button("Gravar Lavados"):
            if c and e: registrar_dados("Lavados", [c, dt, m, p, i, t, e])
...
def pag_secagem(dt):
    ...
    with st.form("f_sec", clear_on_submit=True):
        ...
        if st.form_submit_button("Gravar Secagem"):
            if c and e: registrar_dados("Secagem", [m, c, dt, ent, sai, e])
            else: st.warning("Preencha os campos obrigatórios.")
...
def pag_pesagem(dt):
    ...
    with st.form("f_pes", clear_on_submit=True):
        ...
        if st.form_submit_button("Gravar Pesagem"):
            if c and e: registrar_dados("Pesagem", [c, dt, p, e, tipo])
            else: st.warning("Preencha os campos obrigatórios.")
...
def pag_dobragem(dt):
    ...
    with st.form("f_dob", clear_on_submit=True):
        ...
        if st.form_submit_button("Gravar Dobragem"):
            if c and e: registrar_dados("Dobragem", [c, dt, e] + [int(qtds[it]) for it in ITENS_DOBRAGEM])
            else: st.warning("Preencha Cliente e Executante.")

def pag_analises(dt):
    st.header("📊 Painel Estatístico e Resumos")
    filtro = st.text_input("🔍 Filtrar por Cliente (Vazio para todos)")
    if st.button("Gerar / Atualizar Relatórios"): gerar_relatorios(filtro)
...
def pag_correcoes(dt):
    ...
    if st.button("Visualizar Linhas"): puxar_historico_setor(s)
    st.markdown("---")
    ...
    if st.button("🚨 Apagar Último Registro", disabled=not conf): deletar_ultima_linha(s)
...
# Executa a tela do menu de forma plana (Impossível dar erro de indentação)
